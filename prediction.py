"""
prediction.py  —  ML engine for Trips & Travel Analytics  (v2 — 2-Stage Pipeline)
===================================================================================
STAGE 1 · train_all_models()
    • Cleans & encodes data
    • Applies BorderlineSMOTE + Tomek Links class balancing
    • Trains ALL models with DEFAULT / baseline parameters (no tuning)
    • Evaluates on a held-out test set → produces leaderboard
    • Saves base_model.pkl + base_meta.pkl (+ legacy aliases for compat)

STAGE 2 · tune_best_model()
    • Receives the Stage-1 result dict
    • Runs RandomizedSearchCV only on the Stage-1 winning model
    • Balancing is inside the CV pipeline (no leakage)
    • Saves tuned_model.pkl + tuned_meta.pkl

Prediction runtime always prefers the tuned model when available.
"""
...
# ─────────────────────────────────────────────────────────────────
# ARTEFACT PATHS  — v2 layout (+ legacy aliases)
# ─────────────────────────────────────────────────────────────────
ARTEFACT_DIR = "."

# Stage-1  (baseline winner)
BASE_MODEL_PATH = os.path.join(ARTEFACT_DIR, "base_model.pkl")
BASE_META_PATH  = os.path.join(ARTEFACT_DIR, "base_meta.pkl")

# Stage-2  (tuned winner — written only after tuning)
TUNED_MODEL_PATH = os.path.join(ARTEFACT_DIR, "tuned_model.pkl")
TUNED_META_PATH  = os.path.join(ARTEFACT_DIR, "tuned_meta.pkl")

# Shared encoding / scaling artefacts (written once by Stage-1, reused by Stage-2)
...
# Legacy aliases — kept so old saved files still load without errors
BEST_MODEL_PATH = BASE_MODEL_PATH   # maps to base model
TRAIN_META_PATH = BASE_META_PATH    # maps to base meta
...
TUNING_RATIONALE_TEMPLATE = (
    "**Why tune {model_name}?**  \n\n"
    "The baseline {model_name} was selected as the best base learner by F1 score. "
    "Stage 2 runs `RandomizedSearchCV` (F1 scoring, 5-fold StratifiedKFold) with "
    "BorderlineSMOTE + Tomek Links *inside* each CV fold to remain leakage-safe. "
    "This targeted search finds better hyperparameters without exhaustive grid search, "
    "trading small precision changes for improved recall on the minority class."
)
...
# ─────────────────────────────────────────────────────────────────
# BALANCING  (training-time only — never applied to test data)
# ─────────────────────────────────────────────────────────────────
def apply_balancing(X_train, y_train):
    """Apply BorderlineSMOTE + Tomek Links to training fold only."""
    if not HAS_IMBLEARN:
        return X_train, y_train, "None (imblearn not installed)"
    try:
        pipe = ImbPipeline(_sampler_steps())
        Xr, yr = pipe.fit_resample(X_train, y_train)
...
def _sampler_steps():
    """Return the (name, sampler) steps for BorderlineSMOTE + TomekLinks.
    Returned as a list so callers can splice them directly into an ImbPipeline
    instead of nesting a pipeline-inside-a-pipeline."""
    return [
        ("oversample", BorderlineSMOTE(random_state=42, k_neighbors=5)),
        ("undersample", TomekLinks()),
    ]


def _build_cv_pipeline(model_name: str, estimator):
    """
    Build an imblearn / sklearn pipeline suitable for CV.
    Balancing is INSIDE the pipeline so it only sees training folds — no leakage.
    Returns (pipeline, param_prefix_string).
    """
    model = clone(estimator)
    if model_name == "Logistic Regression":
        if HAS_IMBLEARN:
            return ImbPipeline(
                [("scaler", StandardScaler())] + _sampler_steps() + [("model", model)]
            ), "model__"
        return SkPipeline([
    ...
    if HAS_IMBLEARN:
        return ImbPipeline(
            _sampler_steps() + [("model", model)]
        ), "model__"
    return model, ""


def _prefix_params(params, prefix):
    if not prefix:
        return params
...
# ─────────────────────────────────────────────────────────────────
# MODEL REGISTRY  — baseline defaults only
# ─────────────────────────────────────────────────────────────────
def _model_registry():
    """
    Returns {name: (baseline_estimator, tuning_search_space)}.
    Stage 1 uses only the estimator (default params).
    Stage 2 uses the search space for the single winning model.
    """
    models = {
        "Logistic Regression": (
...
# ─────────────────────────────────────────────────────────────────
# ══════════════════════════════════════════════════════════════════
# STAGE 1 · train_all_models  (baseline, no tuning)
# ══════════════════════════════════════════════════════════════════
# ─────────────────────────────────────────────────────────────────
def train_all_models(df: pd.DataFrame, progress_cb=None) -> dict:
    """
    Stage 1 — Baseline multi-model training.

    Pipeline (leakage-safe):
      1. Encode categoricals → stratified train/test split (80/20)
      2. Fit StandardScaler on train only
      3. Apply BorderlineSMOTE + Tomek Links to TRAINING set only
      4. Fit each model with DEFAULT parameters on the balanced training set
      5. Evaluate all models on the untouched test set
      6. Rank by F1 → identify best base learner

    Returns a result dict consumed by tune_best_model() and save_base_artefacts().
    Does NOT perform any hyperparameter search.
    """
    ...
    # Scaler fit on raw train — used by Logistic Regression
    scaler     = StandardScaler()
    ...
    # Balance training set (raw + scaled versions for LR vs tree models)
    ...
    for name, (base, _) in registry.items():           # ← ignore search space here
        if progress_cb:
            progress_cb(name)
        ...
        try:
            model = clone(base)                        # ← pure default params
            model.fit(X_tr, y_tr)
        except Exception as e:
        ...
        # Cross-validated ROC-AUC (inside CV pipeline — leakage-safe)
        try:
            cv_pipe, _ = _build_cv_pipeline(name, base)
            cv = StratifiedKFold(n_splits=5, shuffle=True, random_state=42)
            cvr = cross_val_score(cv_pipe, X_train, y_train,
    ...
    return {
        # ── model artefacts ──────────────────────────────────────
        "models"          : trained,
        "best_model"      : best_model,       # best baseline model
        "best_name"       : best_name,
        # ── encoding / scaling ───────────────────────────────────
        "scaler"          : scaler,
        "le_dict"         : le_dict,
        ...
        # ── raw train set  (needed by Stage-2 tune_best_model) ───
        "X_train_raw"     : X_train,
        "y_train_raw"     : y_train,
        # ── balancing metadata ───────────────────────────────────
        "balance_method"  : bm,
...
# ─────────────────────────────────────────────────────────────────
# ══════════════════════════════════════════════════════════════════
# STAGE 2 · tune_best_model  (single-model tuning)
# ══════════════════════════════════════════════════════════════════
# ─────────────────────────────────────────────────────────────────
def tune_best_model(base_result: dict, tune_iters: int = 25,
                    progress_cb=None) -> dict:
    """
    Stage 2 — Hyperparameter tuning for the Stage-1 winning model only.

    Pipeline (leakage-safe):
      • RandomizedSearchCV with StratifiedKFold(5)
      • Balancing is INSIDE the CV pipeline → no data leaks into validation folds
      • Refit on the full balanced training set with the best params found
      • Evaluate on the same held-out test set used in Stage 1

    Args:
        base_result : dict returned by train_all_models()
        tune_iters  : number of RandomizedSearchCV iterations
        progress_cb : optional callable(message: str)

    Returns a result dict suitable for save_tuned_artefacts().
    """
    best_name     = base_result["best_name"]
    X_train       = base_result["X_test"]              # ← reconstruct from saved test
    # NOTE: we need the full train set — it's not stored in base_result by default.
    # We store it in base_result starting with this version.
    X_train_raw   = base_result.get("X_train_raw")
    y_train_raw   = base_result.get("y_train_raw")
    scaler        = base_result["scaler"]
    le_dict       = base_result["le_dict"]
    feature_names = base_result["feature_names"]
    X_test        = base_result["X_test"]
    X_test_sc     = base_result["X_test_sc"]
    y_test        = base_result["y_test"]

    registry      = _model_registry()
    if best_name not in registry:
        raise ValueError(f"Model '{best_name}' not found in registry.")

    base_estimator, search_space = registry[best_name]

    # ── baseline metrics (default params on balanced data) ────────
    if progress_cb:
        progress_cb(f"Computing baseline for {best_name}…")

    best_X_test_base = X_test_sc if best_name == "Logistic Regression" else X_test
    baseline_metrics = _eval_model(
        base_result["best_model"],
        best_X_test_base,
        y_test,
        best_name,
    )

    # ── leakage-safe CV search ────────────────────────────────────
    if progress_cb:
        progress_cb(f"Running RandomizedSearchCV on {best_name} ({tune_iters} iters)…")

    cv_pipe, param_prefix = _build_cv_pipeline(best_name, base_estimator)
    cv = StratifiedKFold(n_splits=5, shuffle=True, random_state=42)

    # We need the original (unbalanced, unscaled) training set for CV
    # It must have been stored by the updated train_all_models()
    if X_train_raw is None or y_train_raw is None:
        raise RuntimeError(
            "base_result is missing 'X_train_raw' / 'y_train_raw'. "
            "Please re-run train_all_models() with the updated prediction.py."
        )

    search = RandomizedSearchCV(
        cv_pipe,
        _prefix_params(search_space, param_prefix),
        n_iter=tune_iters,
        cv=cv,
        scoring="f1",
        n_jobs=-1,
    ...
    if progress_cb:
        progress_cb(f"Fitting {tune_iters} candidate configurations…")

    search.fit(X_train_raw, y_train_raw)
    best_params = _strip_prefix(search.best_params_, param_prefix)
    best_cv_f1  = round(search.best_score_, 4)

    # ── refit tuned model on full balanced training data ──────────
    if progress_cb:
        progress_cb(f"Refitting {best_name} with best params on full training set…")

    X_bal, y_bal, _ = apply_balancing(
        X_train_raw if best_name != "Logistic Regression"
        else pd.DataFrame(scaler.transform(X_train_raw), columns=feature_names),
        y_train_raw,
    )

    tuned_model = clone(base_estimator).set_params(**best_params)
    tuned_model.fit(X_bal, y_bal)

    # ── evaluate tuned model ──────────────────────────────────────
    best_X_test_tuned = X_test_sc if best_name == "Logistic Regression" else X_test
    tuned_metrics     = _eval_model(tuned_model, best_X_test_tuned, y_test, best_name)

    # ── build search-space record for UI charts ───────────────────
    tuning_search_space = {}
    for param, values in search_space.items():
        tuning_search_space[param] = {
            "values"   : values,
            "best"     : best_params.get(param, values[0]),
            "rationale": f"Tuned via RandomizedSearchCV ({tune_iters} iters, F1 scoring, 5-fold StratifiedKFold)",
        }

    # ── build baseline-vs-tuned comparison dict ───────────────────
    comparison = {}
    for metric in ["Accuracy", "F1", "ROC-AUC", "Recall", "Precision"]:
        comparison[metric] = {
            "baseline"        : float(baseline_metrics.get(metric, 0) or 0),
            "tuned"           : float(tuned_metrics.get(metric, 0) or 0),
            "higher_is_better": True,
        }

    return {
        # ── tuned model artefacts ─────────────────────────────────
        "tuned_model"        : tuned_model,
        "best_name"          : best_name,
        "best_params"        : best_params,
        "best_cv_f1"         : best_cv_f1,
        # ── shared encoding / scaling ─────────────────────────────
        "scaler"             : scaler,
        "le_dict"            : le_dict,
        "feature_names"      : feature_names,
        # ── evaluation data ───────────────────────────────────────
        ...
        # ── balancing (carried over from base) ────────────────────
        "balance_method"     : base_result["balance_method"],
        "before_balance"     : base_result["before_balance"],
        "after_balance"      : base_result["after_balance"],
        # ── stage flags ───────────────────────────────────────────
        "stage"              : "tuned",
        "tuned"              : True,
    }
...
def save_base_artefacts(result: dict):
    """Persist Stage-1 (baseline) artefacts to disk."""
    with open(BASE_MODEL_PATH,  "wb") as f: pickle.dump(result["best_model"],    f)
    ...
    meta_keys = [
        "best_name", "balance_method", "before_balance", "after_balance",
        "results_df", "feature_names", "y_test",
        "X_test", "X_test_sc", "best_X_test",
        "stage", "tuned",
        "X_train_raw", "y_train_raw",   # needed by Stage-2
    ]
    meta = {k: result[k] for k in meta_keys if k in result}
    with open(BASE_META_PATH,   "wb") as f: pickle.dump(meta, f)


def save_tuned_artefacts(result: dict):
    """Persist Stage-2 (tuned) artefacts to disk. Shared encoders NOT re-saved."""
    with open(TUNED_MODEL_PATH, "wb") as f: pickle.dump(result["tuned_model"], f)
    meta_keys = [
        "best_name", "best_params", "best_cv_f1",
        "balance_method", "before_balance", "after_balance",
        "feature_names", "y_test", "X_test", "X_test_sc", "best_X_test",
        "tuning_search_space", "comparison",
        "baseline_metrics", "tuned_metrics",
        "stage", "tuned",
    ]
    meta = {k: result[k] for k in meta_keys if k in result}
    with open(TUNED_META_PATH,  "wb") as f: pickle.dump(meta, f)


# ── legacy alias (old code that calls save_artefacts still works) ──
def save_artefacts(result: dict):
    save_base_artefacts(result)


def load_base_artefacts():
    """Load Stage-1 artefacts. Returns None if files are missing."""
    try:
        with open(BASE_MODEL_PATH,  "rb") as f: model   = pickle.load(f)
        with open(SCALER_PATH,      "rb") as f: scaler  = pickle.load(f)
        with open(FEAT_NAMES_PATH,  "rb") as f: fnames  = pickle.load(f)
        with open(LABEL_ENC_PATH,   "rb") as f: le      = pickle.load(f)
        with open(BASE_META_PATH,   "rb") as f: meta    = pickle.load(f)
        return {"model": model, "scaler": scaler,
                "feature_names": fnames, "le_dict": le, "meta": meta}
    except FileNotFoundError:
        return None


def load_tuned_artefacts():
    """Load Stage-2 artefacts. Returns None if tuning has not been run."""
    try:
        with open(TUNED_MODEL_PATH, "rb") as f: model   = pickle.load(f)
        with open(SCALER_PATH,      "rb") as f: scaler  = pickle.load(f)
        with open(FEAT_NAMES_PATH,  "rb") as f: fnames  = pickle.load(f)
        with open(LABEL_ENC_PATH,   "rb") as f: le      = pickle.load(f)
        with open(TUNED_META_PATH,  "rb") as f: meta    = pickle.load(f)
        return {"model": model, "scaler": scaler,
                "feature_names": fnames, "le_dict": le, "meta": meta}
    except FileNotFoundError:
        return None
...
def load_production_model():
    """
    Returns the best available model for prediction:
      → tuned model  if Stage-2 has been run
      → base model   if only Stage-1 has been run
      → None         if nothing has been trained yet
    """
    tuned = load_tuned_artefacts()
    if tuned:
        tuned["source"] = "tuned"
        return tuned
    base = load_base_artefacts()
    if base:
...
# ── legacy alias ──
def load_artefacts():
    return load_base_artefacts()


def base_artefacts_exist():
    return all(os.path.exists(p) for p in
               [BASE_MODEL_PATH, SCALER_PATH, FEAT_NAMES_PATH,
                LABEL_ENC_PATH,  BASE_META_PATH])


def tuned_artefacts_exist():
    return os.path.exists(TUNED_MODEL_PATH) and os.path.exists(TUNED_META_PATH)


# ── legacy alias ──
def artefacts_exist():
    return base_artefacts_exist()


# ─────────────────────────────────────────────────────────────────
# CHARTS  —  balance, leaderboard, ROC, CM, PR, radar, dist, FI
# ─────────────────────────────────────────────────────────────────
...
def fig_model_comparison(results_df: pd.DataFrame) -> go.Figure:
    ...
    fig.update_layout(barmode="group",
                      title_text="Baseline Model Comparison — Key Metrics",
...
def fig_roc_curves(models, X_test, y_test, X_test_sc=None) -> go.Figure:
    ...
    fig.update_layout(xaxis_title="FPR", yaxis_title="TPR",
                      title_text="ROC Curves — All Baseline Models",
                      title_font=dict(size=11, color="#EEF2FF"))
    return _theme(fig, 360)
...
def fig_prob_distribution(model, X_test, y_test) -> go.Figure:
    ...
    for val, color, lbl in [(0, "#0D2137", "Not Purchased"), (1, "#00E5A0", "Purchased")]:
        sub = yp[y_arr == val]
        if len(sub):
            fig.add_trace(go.Histogram(x=sub, nbinsx=30, name=lbl,
...
def fig_feature_importance(model, feature_names, model_name) -> go.Figure:
    fi = None
...
# ─────────────────────────────────────────────────────────────────
# TUNING CHARTS  (Stage-2 visualisations)
# ─────────────────────────────────────────────────────────────────
def fig_tuning_comparison(comparison: dict, model_name: str = "Best Model") -> go.Figure:
    """Grouped bar — Baseline vs Tuned for each metric."""
    metrics   = list(comparison.keys())
    baselines = [comparison[m].get("baseline", 0) or 0 for m in metrics]
    tuned     = [comparison[m].get("tuned",    0) or 0 for m in metrics]
    ...
    fig.add_trace(go.Bar(
        ...
        x=metrics, y=tuned,
        marker_color=[
            "#00E5A0" if comparison[m].get("higher_is_better", True) and
                         (comparison[m].get("tuned", 0) or 0) >= (comparison[m].get("baseline") or 0)
    ...
    fig.update_layout(
        barmode="group",
        title=dict(text=f"Baseline vs Tuned {model_name} — Metric Comparison",
...
def fig_tuning_delta(comparison: dict, model_name: str = "Best Model") -> go.Figure:
    """Horizontal bar — metric deltas (Tuned − Baseline)."""
    metrics = list(comparison.keys())
    deltas  = [
        round((comparison[m].get("tuned", 0) or 0) - (comparison[m].get("baseline") or 0), 4)
        for m in metrics
    ]
    ...
    fig.update_layout(
        title=dict(text=f"{model_name} — Metric Delta: Tuned − Baseline  (Green = better, Amber = trade-off)",
                   font=dict(size=11, color="#EEF2FF"), x=0.01),
...
def fig_tuning_search_space(search_space: dict, model_name: str = "Best Model") -> go.Figure:
    """Heatmap of hyperparameter search space with best value highlighted."""
    params   = list(search_space.keys())
    all_vals = sorted(set(
        v for info in search_space.values()
    ...
    fig.update_layout(
        title=dict(text=f"{model_name} Search Space  |  ★ = Best Value Selected",
                   font=dict(size=11, color="#EEF2FF"), x=0.01),
...
def fig_best_params_bar(search_space: dict, model_name: str = "Best Model") -> go.Figure:
    """Horizontal bar of best parameter values (normalised 0-1)."""
    params    = list(search_space.keys())
    best_vals = [search_space[p]["best"] for p in params]
    max_vals  = [
        max((v for v in search_space[p]["values"] if isinstance(v, (int, float))), default=1)
        for p in params
    ]
    ...
    fig.update_layout(
        title=dict(text=f"Best Parameters Selected for {model_name} (Tuned)",
                   font=dict(size=11, color="#EEF2FF"), x=0.01),
...
# TUNING TABLES
# ─────────────────────────────────────────────────────────────────
def get_tuning_summary(search_space: dict) -> pd.DataFrame:
    """DataFrame summarising a model's search space and best values."""
    rows = []
...
def get_tuning_comparison(comparison: dict) -> pd.DataFrame:
    """DataFrame of baseline vs tuned metric comparison."""
    rows = []
    for metric, info in comparison.items():
        baseline = info.get("baseline")
        tuned_v  = info.get("tuned", 0)
        if baseline is None:
            delta_str, direction = "N/A", "➖ No baseline"
        else:
            delta     = round(tuned_v - baseline, 4)
            delta_str = f"{'+' if delta >= 0 else ''}{delta:.4f}"
            direction = ("✅ Improved" if (delta > 0) == info.get("higher_is_better", True)
